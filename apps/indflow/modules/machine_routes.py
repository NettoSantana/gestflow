...
from modules.repos.machine_config_repo import upsert_machine_config, ensure_machine_config_tenant_table
from modules.repos.refugo_repo import load_refugo_24, upsert_refugo
from modules.admin.routes import login_required, admin_required
...
def _load_op_ativa(conn: sqlite3.Connection, machine_id: str, cliente_id: str) -> dict | None:
    cid = str(cliente_id or "").strip()
    mid = _norm_machine_id(str(machine_id or ""))
...
def _ensure_op_bobina_pendencia_table(conn: sqlite3.Connection) -> None:
    conn.execute(
        """
...
def _load_bobina_pendencia(conn: sqlite3.Connection, op_id: int):
    _ensure_op_bobina_pendencia_table(conn)
    row = conn.execute(
        """
        SELECT op_id, machine_id, armed_at, closed_seq, closed_abs_pcs, next_seq
    ...
    return {
        ...
        "machine_id": str(row[1] or "").strip(),
        "armed_at": str(row[2] or "").strip(),
        "closed_seq": int(row[3] or 0),
        "closed_abs_pcs": int(row[4] or 0),
        "next_seq": int(row[5] or 0),
    }

def _clear_bobina_pendencia_conn(conn: sqlite3.Connection, op_id: int) -> None:
    _ensure_op_bobina_pendencia_table(conn)
    conn.execute("DELETE FROM ordens_producao_bobina_pendencia WHERE op_id = ?", (int(op_id),))

def _maybe_open_next_bobina_on_update(
    ...
    - OP ATIVA pertence ao cliente autenticado pela API Key do ESP.
    - Ao clicar 'troca', a rota de producao fecha a bobina atual e grava uma pendencia.
    - No PRIMEIRO /machine/update apos a troca, esta funcao abre a proxima seq
      somente para cliente_id + machine_id e consome a pendencia dessa OP.
    - A nova bobina nasce com start_abs_pcs = closed_abs_pcs da anterior.
      O primeiro pulso apos a troca deve contar na nova bobina, sem descarte.
    - Mantemos o fallback legado por eventos apenas dentro da OP ja isolada pelo tenant.
    """
    cid = str(cliente_id or "").strip()
    mid = _norm_machine_id(str(machine_id or ""))
    if not cid or not mid:
    ...
    op = _load_op_ativa(conn, mid, cid)
    if not op:
        return
    ...
    # Caminho oficial: consumir a pendencia criada em /producao/op/troca-bobina
    pend = _load_bobina_pendencia(conn, op_id)
    if pend:
        pend_mid = _norm_machine_id(str(pend.get("machine_id") or ""))
        if pend_mid and pend_mid != mid:
            return

        try:
            next_seq = int(pend.get("next_seq") or 0)
        except Exception:
            next_seq = 0
        try:
            closed_abs_i = int(pend.get("closed_abs_pcs") or 0)
        except Exception:
            closed_abs_i = 0

        # Se a pendencia estiver invalida ou alem da quantidade de bobinas, limpa para nao travar a OP
        if next_seq < 0 or next_seq >= len(bobinas):
            _clear_bobina_pendencia_conn(conn, op_id)
            return

        # So abre a proxima bobina quando chegar um update real apos o fechamento
        if int(esp_now) <= int(closed_abs_i):
            return

        start_abs_next = int(closed_abs_i)
        ...
        if _event_exists(conn, op_id, next_seq):
            ...
            if _is_event_sem_start(row_next):
                _set_event_start(conn, op_id, next_seq, ts_iso, start_abs_next, created_at)
            # Mesmo que o evento ja exista/preenchido, a pendencia precisa ser consumida
            _clear_bobina_pendencia_conn(conn, op_id)
            return

        _create_bobina_event(conn, op_id, next_seq, bobinas[next_seq], ts_iso, start_abs_next, created_at)
        _clear_bobina_pendencia_conn(conn, op_id)
        return

    # Fallback legado por eventos: mantido apenas para compatibilidade
...
def _machine_config_tenant_upsert(cliente_id: str, machine_id: str, m: dict, cfg_v2: dict) -> bool:
    """
    Dual-write de transicao para a configuracao multiempresa.
    Mantem machine_config legado intacto e grava a copia tenant por
    (cliente_id, machine_id). Retorna False quando nao ha tenant valido.
    """
    cid = str(cliente_id or "").strip()
    mid = _norm_machine_id(_unscope_machine_id(machine_id))
    if not cid or not mid:
    ...
    try:
        if alerta_sec in (None, "", "none"):
            ...
            alerta_sec = max(5, min(86400, int(alerta_sec)))
    except Exception:
        alerta_sec = None

    payload = json.dumps(cfg_v2 or {}, ensure_ascii=True, separators=(",", ":"))
    updated_at = datetime.now(_get_tz()).isoformat(timespec="seconds")
    ...
    try:
        conn.execute(
            """
            INSERT INTO machine_config_tenant
            (
              cliente_id,
              machine_id,
              meta_turno,
...
def _cfgv2_db_load(machine_id: str, cliente_id: str | None = None) -> dict | None:
    """Carrega config_v2 do tenant e usa machine_config legada como fallback de transicao."""
    mid = _norm_machine_id(_unscope_machine_id(machine_id))
    cid = str(cliente_id or "").strip()
    conn = sqlite3.connect(_cfgv2_db_path(), check_same_thread=False)
    try:
        ...
        if not row:
            cur = conn.execute(
                "SELECT config_json FROM machine_config WHERE machine_id = ? LIMIT 1",
                (mid,),
            )
            row = cur.fetchone()
...
def _cfgv2_load_apply(m: dict, machine_id: str, cliente_id: str | None = None) -> None:
    """Recarrega config do tenant; durante a migracao preserva fallback legado."""
    try:
        _cfgv2_db_init()
        if cliente_id:
            ensure_machine_config_tenant_table()
        cfg = _cfgv2_db_load(machine_id, cliente_id)
        if cfg:
...
def configurar_maquina():
    ...
    m = get_machine(machine_id, cliente_id)
    ...
    # Dual-write multiempresa: nova base por cliente + maquina.
    # A leitura continua no legado nesta etapa da migracao.
    try:
        _machine_config_tenant_upsert(cliente_id, machine_id, m, cfg_v2)
    except Exception as e:
        return jsonify({"error": "Falha ao persistir configuracao multiempresa", "detail": str(e)}), 500
...
def _reset_hour_card_on_hour_change(
    m: dict,
    prev_idx,
    prev_data_ref,
    current_idx,
    ...
    """
    Regra do card:
    - SEMPRE que a hora mudar, a producao da hora deve recomecar do zero.
    - Nao depende de meta da hora (pode ser 0 em pausa ou >0 em hora produtiva).
    - Atua apenas no primeiro ciclo da nova hora, para evitar herdar acumulado da hora anterior.
    """
    try:
        curr_idx = int(current_idx) if current_idx is not None else None
    except Exception:
        curr_idx = None
    ...
    try:
        if prev_idx is not None:
            prev_idx_norm = int(prev_idx)
    except Exception:
        prev_idx_norm = None

    curr_day = str(current_data_ref or "").strip()
    prev_day = str(prev_data_ref or "").strip()

    hour_changed = False
    if curr_idx is not None:
        if prev_idx_norm is None:
            hour_changed = False
        elif curr_idx != prev_idx_norm:
            hour_changed = True

    day_changed = bool(curr_day and prev_day and curr_day != prev_day)
    ...
    if curr_idx is not None:
        m["ultima_hora"] = curr_idx
        try:
            prod_h = m.get("producao_por_hora")
            if isinstance(prod_h, list) and 0 <= curr_idx < len(prod_h):
                prod_h[curr_idx] = 0
        except Exception:
            pass

    try:
        exib24 = m.get("producao_exibicao_24")
        if isinstance(exib24, list) and len(exib24) == 24:
            hora_real = None
            try:
                horas_turno = m.get("horas_turno") or []
                if curr_idx is not None and isinstance(horas_turno, list) and 0 <= curr_idx < len(horas_turno):
                    faixa = str(horas_turno[curr_idx])
                    hora_real = int(faixa.split("-", 1)[0].strip().split(":", 1)[0])
            except Exception:
                hora_real = None

            if hora_real is None:
                try:
                    hora_real = int(now_bahia().hour)
                except Exception:
                    hora_real = None

            if hora_real is not None and 0 <= hora_real < 24:
                exib24[hora_real] = 0
    except Exception:
        pass
...
def update_machine():
    ...
    _cfgv2_load_apply(m, machine_id, cliente_id)
    ...
    prev_hour_idx = m.get("_ph_acc_idx", m.get("ultima_hora"))
    prev_hour_day = m.get("_ph_data_ref", m.get("ultimo_dia"))

    atualizar_producao_hora(m)

    _reset_hour_card_on_hour_change(
        m,
        prev_idx=prev_hour_idx,
        prev_data_ref=prev_hour_day,
        current_idx=m.get("_ph_acc_idx", m.get("ultima_hora")),
        current_data_ref=m.get("_ph_data_ref", m.get("ultimo_dia")),
    )
    ...
    try:
        ...
        conn_bob = get_db()
        try:
            _maybe_open_next_bobina_on_update(
                conn_bob,
                machine_id=str(machine_id),
                esp_now=int(esp_now_abs),
                ts_iso=str(ts_iso_evt),
                cliente_id=str(cliente_id),
            )
            conn_bob.commit()
        finally:
...
def admin_reset_hour():
    ...
    m = get_machine(machine_id, cid or None)
    if cid:
        m["cliente_id"] = cid
...
def machine_status():
    ...
    m = get_machine(machine_id, cid_req)

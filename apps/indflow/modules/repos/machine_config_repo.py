# Caminho: C:\Users\vlula\OneDrive\Área de Trabalho\Projetos Backup\GESTFLOW\apps\indflow\modules\repos\machine_config_repo.py
...
def ensure_machine_config_tenant_table():
    """
    Cria a base multiempresa paralela sem substituir machine_config.
    A migracao dos consumidores sera feita gradualmente, um arquivo por vez.
    """
    conn = get_db()
    cur = conn.cursor()
    cur.execute("""
...
def ensure_machine_config_table():
    ...
    # Prepara a estrutura multiempresa sem alterar o comportamento legado.
